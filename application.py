...
def home():
    ...
    all_winners = collection2.find()
    # Filter winners to include only one winner per user
    winners = []
    user_ids_seen = defaultdict(int)
    for winner in all_winners:
        user_id = winner['user_id']
        if user_ids_seen[user_id] == 0:
            winners.append(winner)
            user_ids_seen[user_id] = 1
    # print(winners)
...
#         if user:
#             invite_referral_code = user.get('user_referral_code')
...
#         else:
#             # If user_id is present in session but user is not found, set user to None
#             user = None
#             return render_template('new_profile.html', user=user, loggedin=True)
#     else:
#         # If user_id is not present in session, set user to None
#         user = None
#         return render_template('new_profile.html', user=user, loggedin=True)
...
# ***************************************************Game code Starts*******************************************
# Helper function to get the winner count for each user_id
# Helper function to get the winner details for each user_id
...
def leaderboard():
    winner_counts = get_winner_details()
    return render_template('leaderboard.html', winners=winner_counts)

# @application.route('/leaderboard', methods=["POST", "GET"])
# def leaderboard():
#     # Aggregate the data to calculate the winners' true count for each user
#     pipeline = [
#         {
#             '$match': {'winner': True}  # Filter only the documents with winner: True
#         },
#         {
#             '$group': {
#                 '_id': '$user_id',  # Group by user_id
#                 'count': {'$sum': 1}  # Count the occurrences of winner: True for each user_id
#             }
#         },
#         {
#             '$sort': {'count': -1}  # Sort by the winner count in descending order
#         }
#     ]

#     # Execute the aggregation pipeline
#     winners = list(db.winnersdetails.aggregate(pipeline))
...
def gamepage():
    ...
    # is_game_finished = gamelevel >= 2

    # Initialize disable_button to False by default
    disable_button = False
    # winner_status=False
    # Retrieve the winner_status from the session if it exists
    winner_status = session.get('winner_status')
    print('winner_status is',winner_status)

    if gamedetails:
        latest_gamedetail = gamedetails[-1]
        winner_status = latest_gamedetail.get('winner', False)

        # Daisable the button if the user is not  winner in level 2
    ...
    # if gamelevel == 2:  # and winners_count >= winners_needed:
    #     disable_button = False
    ...
    session['winner_status'] = winner_status

    print(live_players)
    print('usercount is here', len(live_players))

    return render_template("gamepage.html", session=session, level_text=level_text, reset_message=reset_message, gamedetails=gamedetails, disable_button=disable_button, winner_status=winner_status)
...
def next_level():
    ...
    if session['level'] == 2: #and len(live_players) > winners_needed:
...
def shuffle(gamelevel, index):
    ...
    # if gamelevel == 2: #and winners_count >= determine_winner_count(gamelevel):
    #     winner = False
    # else:
...
def get_congratulations_message(level):
    if level == 1:
        ...
        return "Congratulations! You have cleared This level. Proceed to Next level."
    # elif level == 3:
